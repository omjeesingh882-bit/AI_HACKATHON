...
export function formatRelativeDate(dateString?: string | null): string {
  ...
  try {
    ...
    if (diffInDays > 1 && diffInDays < 14) return `in ${diffInDays} days`;
    if (diffInDays < -1 && diffInDays > -14) return `${Math.abs(diffInDays)} days ago`;
...
export function getCategoryColor(category?: string | null): string {
  if (!category) return "bg-gray-100 text-gray-800 dark:bg-gray-800/30 dark:text-gray-300";
  const colors: Record<string, string> = {
    ...
    placement: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300",
  ...
  return colors[key] || "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300";
}